...
st.markdown(
    ...
    [data-testid="stMetric"] {
        ...
        border-bottom: 4px solid #CBD5E1 !important; /* 3D bottom bevel */
...
def calculate_historical_metrics(
    ...
    try:
        ...
        if len(all_dates) >= 2:
            ...
            prev_day_close = float(df[df["date"] == prev_date]["close"].iloc[-1])
            last_day_close = float(df[df["date"] == all_dates[-1]]["close"].iloc[-1])
            day_change_pct = round(((last_day_close - prev_day_close) / prev_day_close) * 100, 2) if prev_day_close else 0.0
        else:
            # Only one day of data — use the day's first open as the prev-close proxy
            # so day_change_pct reflects intraday movement rather than always being 0.
            prev_day_close = float(df["open"].iloc[0]) if not df.empty else float(latest["close"])
...
def calculate_signal(
    ...
    # ── RVOL calculation ───────────────────────────────────────────────────────
    # Numerator depends on market state:
    #   OPEN   → use live WebSocket cumulative day volume (grows through session)
    #   CLOSED → use last completed day's total volume from historical data
    #            (single 1-min candle volume would give ~0.001x, meaningless)
    # Denominator = 20-day average of DAILY volumes (not candle volumes)
    # If avg_volume_20 is 0 (live-quote fallback with no history), RVOL = 0
    # so the RVOL criterion is simply not met — correct behaviour.
    if avg_volume_20 > 0:
        vol_for_rvol = volume if ms["is_open"] else latest_day_volume
        rvol = round(vol_for_rvol / avg_volume_20, 2)
    else:
    ...
    # ── Hard momentum gates ────────────────────────────────────────────────────
    # A stock moving < min_change in either direction cannot be a valid
    # breakout or breakdown regardless of how many structural criteria it meets.
    # Without this gate a flat/sideways stock that happens to sit below VWAP
    # and EMA20 can score 4/5 and show up as SUPER BREAKDOWN.
    ...
    # structural position.  Eliminates sideways stocks sitting below VWAP/EMA
    # being falsely labelled BREAKDOWN.
    ...
    if long_signal:
        ...
        signal = "SHORT"
    elif breakout_score >= min_breakout_score:
        signal = "BREAKOUT"
    elif breakdown_score >= min_breakout_score:
        signal = "BREAKDOWN"
    else:
        return None
...
with st.sidebar:
    ...
    min_breakout_score = st.slider("Breakout Score (out of 6)", 1, 6, 4, 1)
...
def render_index_chart(name, index_token, access_token):
    import sys
    import warnings
    ...
    # WS candles are always fresher — if they overlap with REST, WS wins.
    ...
    if not candles:
        ...
        return
    try:
        # normalize_candles drops volume=0 rows (correct for equities).
        # Indices may legitimately have volume=0 on some candles, so we
        # use a lightweight normalization that keeps all non-NaN rows.
        if candles and isinstance(candles[0], (list, tuple)):
            df = pd.DataFrame(candles,
                              columns=["timestamp", "open", "high", "low", "close", "volume"])
        else:
    ...
    if len(today_df) > 120:
        today_df = today_df.iloc[-120:].reset_index(drop=True)
    ...
    # Ensure float64 columns — avoid Python 3.11 Generic[] TypeError in plotly
...
def render_candlestick_chart(stock_name: str, stock_token: str, signal_type: str, live_quote: dict | None = None) -> None:
    import sys
    import warnings
    sys.modules['warnings'] = warnings
    ...
    if len(cdf) > 120:
        cdf = cdf.iloc[-120:].reset_index(drop=True)
    ...
    # that occurs in Python 3.11 when pandas Series have mixed dtypes.
    fig.add_trace(go.Candlestick(
        x=cdf["dt"].tolist(),
...
@st.fragment(run_every=3)
def index_charts_fragment(access_token: str) -> None:
    ...
    with _ic1:
        render_index_chart("NIFTY 50",   "NIDX:40000001", access_token)
    with _ic2:
        render_index_chart("BANK NIFTY", "NIDX:40000003", access_token)
...
def live_scanner_fragment(
    ...
    if True:
        ...
        super_bo_df  = df[((df["Signal"] == "BREAKOUT") | (df["Signal"] == "LONG"))  & (df["_bo_score"] == 6)] if not df.empty else pd.DataFrame(columns=DISPLAY_COLS)
        reg_bo_df    = df[((df["Signal"] == "BREAKOUT") | (df["Signal"] == "LONG"))  & (df["_bo_score"] < 6)]  if not df.empty else pd.DataFrame(columns=DISPLAY_COLS)
        super_bd_df  = df[((df["Signal"] == "BREAKDOWN") | (df["Signal"] == "SHORT")) & (df["_bd_score"] == 6)] if not df.empty else pd.DataFrame(columns=DISPLAY_COLS)
        reg_bd_df    = df[((df["Signal"] == "BREAKDOWN") | (df["Signal"] == "SHORT")) & (df["_bd_score"] < 6)]  if not df.empty else pd.DataFrame(columns=DISPLAY_COLS)
        ...
        _bo_label    = "BO Watchlist"     if _closed else "Super Breakout"
        _bd_label    = "BD Watchlist"     if _closed else "Super Breakdown"
        _rbo_label   = "Breakout Setup"   if _closed else "Breakout"
        _rbd_label   = "Breakdown Setup"  if _closed else "Breakdown"
        _tab_sbo     = "📋 BO Setup"       if _closed else "⚡ Super Breakout"
        _tab_sbd     = "📋 BD Setup"       if _closed else "⚡ Super Breakdown"
        _tab_bo      = "📈 Breakout Setup"  if _closed else "📈 Breakout"
        _tab_bd      = "📉 Breakdown Setup" if _closed else "📉 Breakdown"

        mc = st.columns(7)
        mc[0].metric(_sig_label,  len(df))
        mc[1].metric("LONG",      len(long_df))
        mc[2].metric("SHORT",     len(short_df))
        mc[3].metric(_bo_label,   len(super_bo_df))
        mc[4].metric(_bd_label,   len(super_bd_df))
        mc[5].metric(_rbo_label,  len(reg_bo_df))
        mc[6].metric(_rbd_label,  len(reg_bd_df))

        st.divider()

        t_sbo, t_sbd, t_bo, t_bd, t_long, t_short, t_all, t_log = st.tabs([
            _tab_sbo, _tab_sbd, _tab_bo, _tab_bd,
        ...
        if _ms2["is_open"]:
            ...
            _curr_super = len(super_bo_df) + len(super_bd_df)
            if _curr_super > _prev_super:
                for _, _row in pd.concat([super_bo_df, super_bd_df]).iterrows():
                    st.toast(
        ...
        def show_charts(d: pd.DataFrame, sig_type: str) -> None:
            ...
            for i, row in enumerate(d.itertuples()):
                ...
                with cols[i % len(cols)]:
                    render_candlestick_chart(row.Stock, token, sig_type, live_quote=raw_quote)

        with t_sbo:
            st.subheader("⚡ SUPER BREAKOUT — All 6 Criteria Met")
            if super_bo_df.empty:
                st.info("No Super Breakout stocks at the current filter settings.")
            else:
                show_table(super_bo_df)
                st.markdown("#### Intraday Charts")
                show_charts(super_bo_df, "BREAKOUT")

        with t_sbd:
            st.subheader("⚡ SUPER BREAKDOWN — All 6 Criteria Met")
            if super_bd_df.empty:
                st.info("No Super Breakdown stocks at the current filter settings.")
            else:
                show_table(super_bd_df)
                st.markdown("#### Intraday Charts")
                show_charts(super_bd_df, "BREAKDOWN")

        with t_bo:
            st.markdown('<span class="sig-badge sig-bo">\u2191 BREAKOUT (Partial)</span>', unsafe_allow_html=True)
            if reg_bo_df.empty:
                st.info("No partial Breakout stocks.")
            else:
                show_table(reg_bo_df)
                st.markdown("#### Intraday Charts")
                show_charts(reg_bo_df, "BREAKOUT")

        with t_bd:
            st.markdown('<span class="sig-badge sig-bd">\u2193 BREAKDOWN (Partial)</span>', unsafe_allow_html=True)
            if reg_bd_df.empty:
                st.info("No partial Breakdown stocks.")
            else:
                show_table(reg_bd_df)
                st.markdown("#### Intraday Charts")
                show_charts(reg_bd_df, "BREAKDOWN")
        ...
        with t_log:
            ...
            if st.session_state.signal_log:
                ...
                def _style_log(row: pd.Series) -> list[str]:
                    ...
                    if "SUPER" in sig or row.get("Score", "") == "6/6":
                        return ["background:rgba(251,191,36,0.18);color:#7c5200;font-weight:700"] * len(row)
                    if "BREAKOUT" in sig:
...
if st.session_state.token_accepted:
    ...
    with _tab_indices:
        index_charts_fragment(access_token=access_token)
